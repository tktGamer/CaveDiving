...
float4 main(PS_IN input) : SV_TARGET
{
    ...
    //黒の場所は色を変える
    bool isBlack = all(output.xyz == float3(0.0f,0.0f,0.0f));
    
    float3 finalColor = isBlack ? color.xyz : output.xyz;
    ...
    for (int i = 0; i < onLightCount; i++)
    {
        ...
        //ライトからの距離を考慮した色
        float3 tem = finalColor * lights[i].LightColor * lights[i].LightIntensity * ToonStep(intensity);
        //tem = diffuse;
        temTotal += tem;
    }
    
    //元の色より明るくしないようにする
    output.xyz = min(temTotal, finalColor.xyz);

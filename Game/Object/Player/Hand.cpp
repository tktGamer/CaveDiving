...
{
	SetQuaternion(GetQuaternion() * DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(DirectX::SimpleMath::Vector3::UnitY, DirectX::XMConvertToRadians(1.0f)));
	m_currentAngle = currentAngle * GetQuaternion();
	m_currentPosition = DirectX::SimpleMath::Vector3::Transform(GetPosition(), m_currentAngle ) + currentPosition;
}

...
void GolemSlammedDownPreparingMotion::Initialize()
{
	...
	m_pLeftGolemHand->SetQuaternion(DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(DirectX::SimpleMath::Vector3::UnitX, DirectX::XMConvertToRadians(90.0f)));


	//スタート位置とゴール位置
	m_rightHandStartPosition = m_pRightGolemHand->GetPosition();
	m_rightHandGoalPosition = m_pRightGolemHand->GetPosition() + DirectX::SimpleMath::Vector3{ -1.0f,6.0f,-6.0f };
	m_leftHandStartPosition = m_pLeftGolemHand->GetPosition();
	m_leftHandGoalPosition = m_pLeftGolemHand->GetPosition() + DirectX::SimpleMath::Vector3{ 1.0f,6.0f,-6.0f };
}
